...
def rsi_strat(commodity_data_df, price_col, window=14, overbought_threshold=70, oversold_threshold=30):
    ...
    rsi_df['Position'] = np.where((rsi_df['RSI'] < oversold_threshold) & (rsi_df['RSI'].shift(1) < rsi_df['RSI']) & (rsi_df[price_col].shift(1) > rsi_df[price_col]), 1, 0)
    rsi_df['Position'] = np.where((rsi_df['RSI'] > overbought_threshold) & (rsi_df['RSI'].shift(1) > rsi_df['RSI']) & (rsi_df[price_col].shift(1) < rsi_df[price_col]), -1, rsi_df['Position'])
