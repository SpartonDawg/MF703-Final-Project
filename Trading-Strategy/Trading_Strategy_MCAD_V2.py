...
def MACD_strat(commodity_data_df, price_col, short_window=12, long_window=26, signal_window=9):
    #This version includes the analysis of divergence between the price and MCAD
    ...
    mcad_df['Price_Swing'] = mcad_df[price_col].diff()
    mcad_df['Bullish_Divergence'] = np.where((mcad_df['Price_Swing'] < 0) & (mcad_df['MACD'].diff() > 0), 1, 0)
    mcad_df['Bearish_Divergence'] = np.where((mcad_df['Price_Swing'] > 0) & (mcad_df['MACD'].diff() < 0), 1, 0)
    
    
    mcad_df['Position'] = np.where((mcad_df['MACD'] > mcad_df['Signal_Line']) & (mcad_df['MACD'].shift(1) <= mcad_df['Signal_Line'].shift(1)) & (mcad_df['Bullish_Divergence'] > 0), 1, 0)
    mcad_df['Position'] = np.where((mcad_df['MACD'] < mcad_df['Signal_Line']) & (mcad_df['MACD'].shift(1) >= mcad_df['Signal_Line'].shift(1)) & (mcad_df['Bearish_Divergence'] > 0), -1, mcad_df['Position'])
