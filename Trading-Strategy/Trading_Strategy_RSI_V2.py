...
def rsi_strat(commodity_data_df, price_col, window=14, vol_window=20, overbought_multiplier=1.5, oversold_multiplier=0.50):
    ...
    volatility = rsi_df['RSI'].diff().abs().rolling(window=vol_window, min_periods=1).mean()
    overbought_threshold = rsi_df['RSI'].mean() + (overbought_multiplier * volatility)
    oversold_threshold = rsi_df['RSI'].mean() - (oversold_multiplier * volatility)
