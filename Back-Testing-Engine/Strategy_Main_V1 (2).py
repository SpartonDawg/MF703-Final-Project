...
def Back_Tester(weight_type):
    ...
    #positions_df = breakout_strat(commodity_prices_df, atr_window, avg_window)
    #positions_df.to_csv('positions_data_Breakout.csv', index=True)  
    ...
    #scaled_daily_returns_df.to_csv('scaled_returns_data_AQR.csv', index=True)  
    
    equal_weights = generate_equal_weights(positions_df,scaled_daily_returns_df)
    #equal_weights.to_csv("eql_weights_Breakout.csv")
    ...
    #covar_df.to_csv('position_weights_covar_data_Breakout.csv', index=True)
    ...
    if weight_type == 1:
        portfolio_returns_df = returns_rebalancing(scaled_daily_returns_df.iloc[1:], equal_weights, rebalancing_dates_df.iloc[1:], transaction_costs = 0.0005)
    #portfolio_returns_df.to_csv('portfolio_returns_equal_data_Breakout.csv', index=True) 
...
results = Back_Tester(weight_type = 0)
plt.plot(pd.to_datetime(results['Date']),(1+results['Portfolio Return']).cumprod()-1,label="AQR Optimized Weights")
    
results = Back_Tester(weight_type = 1)
plt.plot(pd.to_datetime(results['Date']),(1+results['Portfolio Return']).cumprod()-1,label="AQR Equal Weights")
...
plt.title("AQR Trading Strategy Returns w/ Transaction Costs", size = 22)
